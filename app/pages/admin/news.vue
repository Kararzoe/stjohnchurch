      ...
      <div class="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div class="p-6 space-y-4">
          ...
          <div>
            ...
            <input v-model="form.excerpt" type="text" placeholder="Short description"
              class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-gold/40 focus:border-gold transition-all" />
          </div>
          <div>
            <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Content</label>
          ...
          <div>
            ...
            <input type="file" accept="image/*" @change="handleImageUpload" class="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-bold file:bg-navy/5 file:text-navy hover:file:bg-navy/10 transition-all" />
          ...
          <p v-if="formError" class="text-red-500 text-xs bg-red-50 rounded-xl p-3">{{ formError }}</p>
...
const form = reactive({ title: '', excerpt: '', content: '', image_url: '', published: false })
...
function openForm(post?: any) {
  ...
  formError.value = ''
  if (post) {
    Object.assign(form, { title: post.title, excerpt: post.excerpt ?? '', content: post.content ?? '', image_url: post.image_url ?? '', published: post.published })
  } else {
    Object.assign(form, { title: '', excerpt: '', content: '', image_url: '', published: false })
  }
  showForm.value = true
}
...
async function savePost() {
  ...
  if (editing.value) {
    await supabase.from('news').update(payload).eq('id', editing.value.id)
  } else {
